...
import { 
  ClipboardList, Clock, Wrench, Building, Users, Calendar, Timer, Zap, CalendarDays, AlertCircle, TrendingDown, Filter, X, Package, MessageCircle, ChevronRight, Percent
} from 'lucide-react';
...
const ServiceOrdersPage: React.FC<ServiceOrdersProps> = ({ osData: data, inventoryData, isLoading }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {/* MODAL PRINCIPAL: PEÇAS POR EQUIPAMENTO */}
            ...
            <div className="p-0 overflow-hidden">
              <div className="max-h-[60vh] overflow-y-auto">
                {equipmentParts.length > 0 ? (
                  <table className="w-full text-sm text-left">
                    <thead className="text-[10px] uppercase bg-slate-50 dark:bg-slate-800 text-slate-400 font-bold sticky top-0 z-10">
                      <tr>
                        <th className="px-6 py-3">Descrição</th>
                        <th className="px-6 py-3 text-right">Frequência em OS</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {equipmentParts.map((item, idx) => (
                        <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                          ...
                          <td className="px-6 py-4 text-right">
                            <button 
                              onClick={() => setSelectedPartForReasons({ displayName: item.displayName, reasons: item.reasons })}
                              className="inline-flex items-center px-3 py-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-400 font-bold text-xs transition-colors group"
                            >
                              <div className="flex flex-col items-end mr-3">
                                <span>{item.quantidadeOS} ocorrência(s)</span>
                                <span className="text-[10px] text-blue-500/80 flex items-center">
                                  <Percent className="w-2.5 h-2.5 mr-0.5" />
                                  {item.percentage}%
                                </span>
                              </div>
                              <MessageCircle className="w-3.5 h-3.5 opacity-50 group-hover:opacity-100 transition-opacity" />
                              <ChevronRight className="w-3 h-3 ml-1 opacity-0 group-hover:opacity-100 -translate-x-1 group-hover:translate-x-0 transition-all" />
                            </button>
                          </td>
                        </tr>
                      ))}
                  ...
                  <div className="py-16 flex flex-col items-center justify-center text-slate-400 bg-slate-50/30 dark:bg-slate-900/10">
