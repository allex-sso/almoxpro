...
const WarehouseAddresses: React.FC<WarehouseAddressesProps> = ({ addresses, inventoryData = [], movements = [], isLoading }) => {
  ...
  const [locationFilter, setLocationFilter] = useState<'Todos' | 'Estoque' | 'Picking'>('Todos');
  ...
  // Mapa de Endereço -> Itens (Otimizado)
  const addressToItemsMap = useMemo(() => {
    const locMap: Record<string, InventoryItem[]> = {};
    const itemByCode: Record<string, InventoryItem> = {};
    
    inventoryData.forEach(item => {
      itemByCode[item.codigo] = item;
    ...
    const finalMap: Record<string, InventoryItem[]> = {};
    addresses.forEach(a => {
      const addrKey = a.endereco.trim();
      if (a.codigoCadastrado && itemByCode[a.codigoCadastrado]) {
        finalMap[addrKey] = [itemByCode[a.codigoCadastrado]];
      } else if (locMap[addrKey]) {
        finalMap[addrKey] = locMap[addrKey];
      }
    });
    
    return finalMap;
  }, [inventoryData, addresses]);

  // Mapa de Endereço -> Tipo (Estoque/Picking) baseado na planilha e movimentos
  const addressTypeMap = useMemo(() => {
    const map: Record<string, string> = {};
    
    // 0. Inicializar com tipos da planilha de endereços (Coluna "Localização")
    addresses.forEach(a => {
      if (a.tipo) {
        map[a.endereco.trim()] = a.tipo;
      }
    });
  ...
  const filteredAddresses = useMemo(() => {
    ...
    return addresses.filter(a => {
      ...
      const addrType = addressTypeMap[a.endereco.trim()] || '';
      const matchesLocation = locationFilter === 'Todos' || 
        (locationFilter === 'Estoque' && addrType.toLowerCase().includes('estoque')) ||
        (locationFilter === 'Picking' && addrType.toLowerCase().includes('picking'));
      
      return matchesSearch && matchesOccupation && matchesLocation;
    });
  }, [addresses, searchTerm, occupationFilter, locationFilter, addressTypeMap]);
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-white dark:bg-dark-card p-4 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-800 flex flex-col md:flex-row gap-4 items-center no-print">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 px-4 py-2 rounded-2xl border border-slate-200 dark:border-slate-700 flex items-center gap-3 relative group">
          ...
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Localização:</span>
          <div className="relative flex items-center">
            <select 
            ...
            >
              <option value="Todos" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Todos</option>
              <option value="Estoque" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Estoque</option>
              <option value="Picking" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Picking</option>
            </select>
            <ChevronDown className="w-3.5 h-3.5 text-slate-400 absolute right-0 pointer-events-none" />
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-4 no-print">
        {paginatedData.map(a => {
          ...
          const itemsInAddress = addressToItemsMap[a.endereco.trim()] || [];
          ...
          return (
            ...
            >
               {/* Tooltip Customizado */}
               {hoveredAddress === a.id && itemsInAddress.length > 0 && (
                         ...
                         <div key={item.id} className={idx > 0 ? 'pt-3 border-t border-slate-700/50' : ''}>
                            ...
                            <div className="mt-3 pt-3 border-t border-slate-700/50 flex justify-between items-end">
                               ...
                               <div className="flex flex-col items-end">
                                  <span className="text-sm font-black text-white">{item.quantidadeAtual.toLocaleString('pt-BR')}</span>
                                  {addressTypeMap[a.endereco.trim()] && (
                                    <span className="text-[8px] font-bold text-slate-500 uppercase mt-0.5">{addressTypeMap[a.endereco.trim()]}</span>
                                  )}
                               </div>
                            </div>
                         </div>
